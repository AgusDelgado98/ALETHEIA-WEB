...
describe("reconciliación editorial WEB-2R", () => {
  it("312 textos: la suma de veredictos es exacta", () => {
    const sum = Object.values(inv.verdicts.by_verdict).reduce((a, b) => a + b, 0);
    expect(inv.verdicts.total_units).toBe(312);
    expect(sum).toBe(312);
  });
  it("18 public_question APPROVED; 294 PENDING_AUTHOR_REVIEW; 0 REVISED/REJECTED", () => {
    ...
    expect(inv.verdicts.by_verdict.PENDING_AUTHOR_REVIEW).toBe(294);
